...
ws.addEventListener("message", async ({ data }) => {
	...
	const id = series[sIndex];
	const file = join(outDir, id, dIndex + ".dicom");
	...
	if (dIndex >= series_dicom_number[id]) {
		dIndex = 0;
		sIndex++;
		...
		if (series[sIndex].startsWith("dfyfilm")) {
			sIndex += 1;
		}
		if (sIndex >= series.length) {
			return ws.close();
	...
	requestDICOM(series[sIndex], ++dIndex);
});
